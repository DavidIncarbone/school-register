...
export const AttendanceFormPage = () => {
  ...
  const forcedCourseId = location?.forcedCourseId ?? null;
  const forcedTakeAttendance = location?.forcedTakeAttendance ?? null;
  ...
  return (
    <div>
      ...
      {students ? (
        ...
        <pre className="py-4 text-4xl text-amber-500">appello gia eseguito</pre>
      )}
      {/* take attendance */}
      {students ? (
                ...
                <div>
                  <input
                    onChange={() =>
                      setSelected((prev) =>
                        prev?.map((checked, i) =>
                          i === studentIndex ? !checked : checked
                        )
                      )
                    }
                    className="size-4"
            ...
            {presences &&
              presences.data.map((student) => (
                ...
                >
                  ...
                  <span>{String(Boolean(student.is_present))}</span>
                </div>
              ))}
          </div>
        </>
